...
import { ArrowLeft, Save, Loader2, Layout, Quote, BarChart3, Users, Image as ImageIcon, Building2, Plus, Trash2, Upload } from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#F8FAFC] pb-20">
      <div className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b p-4 flex justify-between items-center px-10">
        <div className="flex items-center gap-4">
          ...
          <h1 className="font-black text-slate-800 tracking-tight text-xl">DASHBOARD EDITOR</h1>
        </div>
      ...
      <div className="max-w-4xl mx-auto py-12 space-y-10 px-6">
        ...
        <div className="bg-white p-8 rounded-[32px] shadow-sm border border-slate-100">
          ...
          />
          <textarea className="w-full text-lg text-slate-500 outline-none h-24 resize-none leading-relaxed" value={attr.hero?.subtitle || ""} 
          ...
          <div className="grid grid-cols-2 gap-4">
            {attr.stats?.map((s: any, i: number) => (
              <div key={i} className="bg-slate-50 p-4 rounded-2xl border flex flex-col relative group">
                ...
                />
                <input className="w-full text-[10px] font-bold text-slate-400 bg-transparent outline-none uppercase" value={s.label}
                  onChange={(e) => {
        ...
        <div className="bg-white p-8 rounded-[32px] shadow-sm border border-slate-100">
          ...
          <div className="space-y-4 mb-6">
            <input className="w-full text-2xl font-black outline-none" placeholder="Chairperson Name" value={attr.chairMessage?.name || ""} 
              onChange={(e) => setData({...data, attributes: {...attr, chairMessage: {...attr.chairMessage, name: e.target.value}}})} 
            />
            <input className="w-full text-md font-bold text-blue-500 outline-none border-b border-slate-50" placeholder="Professional Title" value={attr.chairMessage?.role || ""} 
              onChange={(e) => setData({...data, attributes: {...attr, chairMessage: {...attr.chairMessage, role: e.target.value}}})} 
            />
          </div>
          <textarea className="w-full bg-slate-50 p-6 rounded-2xl outline-none h-48 resize-none leading-relaxed text-slate-600" value={attr.chairMessage?.fullMessage || ""} 
          ...
          <div className="grid grid-cols-4 gap-4">
            {attr.partnerlogo?.map((p: any, i: number) => (
              <div key={i} className="group relative flex flex-col gap-2">
                <div className="w-full h-20 bg-slate-50 rounded-xl border-2 border-dashed border-slate-200 flex items-center justify-center p-2">
                    {p.logo ? (
