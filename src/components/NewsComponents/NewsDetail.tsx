  ...
  if (error || !article) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-6 text-center">
        <h2 className="text-3xl md:text-5xl font-black text-slate-900 mb-6">Article Not Found</h2>
        <button onClick={() => navigate('/news')} className="flex items-center gap-2 bg-[#5F1C9F] text-white px-8 py-4 rounded-full font-black text-[10px] uppercase">
    ...
    <main className="bg-white min-h-screen font-sans animate-fade-in-up overflow-x-hidden">
      <nav className="sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-gray-50 px-4 py-4 md:px-10">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
             <span className="hidden md:block text-[9px] font-black text-gray-300 uppercase tracking-widest">APF Uganda News</span>
      ...
      <header className="max-w-4xl mx-auto pt-10 md:pt-20 px-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <span className="text-[#5F1C9F] font-black text-[10px] uppercase tracking-[0.4em]">{article.category || 'Featured'}</span>
        </div>
        <h1 className="text-3xl md:text-6xl font-black text-slate-900 leading-[1.1] tracking-tighter mb-10">{article.title}</h1>
          ...
          {Array.isArray(mainContent) && mainContent.length > 0 ? (
            mainContent.map((block: any, i: number) => {
              ...
              if (block.type === 'heading') {
                ...
                const headValue = block.value || block.children?.map((c: any) => c.text).join('') || "";
                return <HeadingTag key={i} className="font-black text-slate-900 mt-12 mb-6">{headValue}</HeadingTag>;
              }
              // HANDLE IMAGES INSIDE CONTENT
              if (block.type === 'image' && block.value) {
                return <img key={i} src={block.value} className="w-full rounded-2xl my-10 shadow-lg" alt="" />;
              }
              return null;
            })
          ) : (
           
            <p className="text-xl md:text-2xl font-bold leading-snug text-slate-900">
              {article.description || article.summary || "No content available for this article."}
            </p>
          )}
        </div>
      </article>
