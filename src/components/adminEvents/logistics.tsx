...
export const LogisticsSidebar = ({ data, onChange }: any) => {
  ...
  return (
    <div className="bg-white p-4 md:p-6 lg:p-8 rounded-2xl md:rounded-3xl lg:rounded-[40px] border border-slate-100 shadow-sm space-y-6 md:space-y-8 font-sans">
      <h3 className="font-bold text-slate-800 flex items-center gap-2 text-sm md:text-base uppercase tracking-widest">
        <Sparkles size={16} className="md:w-5 md:h-5 text-purple-500" /> Logistics
      </h3>

      <div className="space-y-4 md:space-y-6">
        {/* Start Date & Time */}
        <div className="space-y-2">
          <label className="text-xs md:text-sm font-black text-slate-400 uppercase ml-1 flex items-center gap-1">
          ...
          <div className="grid grid-cols-2 gap-2">
            <input 
              type="date" 
              min={today} 
              className="bg-slate-50 border-none rounded-xl md:rounded-2xl px-3 md:px-4 py-2.5 md:py-3 text-xs md:text-sm font-bold text-slate-700 outline-none focus:ring-2 focus:ring-purple-100 transition" 
      ...
      <div className="space-y-3 md:space-y-4">
        <label className="text-xs md:text-sm font-black text-slate-400 uppercase ml-1">Venue & Type</label>
        <div className="flex bg-slate-100 p-1 rounded-xl md:rounded-2xl">
          <button 
            type="button"
            onClick={() => onChange('isVirtual', false)}
            className={`flex-1 py-2 md:py-2.5 rounded-lg md:rounded-xl text-xs md:text-sm font-black transition-all ${!data.isVirtual ? 'bg-white text-[#5C32A3] shadow-sm' : 'text-slate-400'}`}
          >PHYSICAL</button>
          <button 
            type="button"
        ...
        <div className="relative">
          <div className="absolute left-3 md:left-4 top-1/2 -translate-y-1/2 text-slate-300">
      ...
      <div className="space-y-3 md:space-y-4 pt-4 border-t border-slate-50">
        <div className="flex justify-between items-center px-1">
          <label className="text-xs md:text-sm font-black text-slate-400 uppercase flex items-center gap-1">
            <CreditCard size={12} className="md:w-3.5 md:h-3.5" /> Payment
          </label>
          <div className="flex items-center gap-2">
             <span className={`text-xs md:text-sm font-black ${!data.isPaid ? 'text-green-500' : 'text-slate-300'}`}>FREE</span>
             <button 
                type="button"
                onClick={() => onChange('isPaid', !data.isPaid)}
                className={`w-10 md:w-12 h-5 md:h-6 rounded-full relative transition-colors ${data.isPaid ? 'bg-purple-600' : 'bg-slate-200'}`}
             >
             ...
             <span className={`text-xs md:text-sm font-black ${data.isPaid ? 'text-purple-600' : 'text-slate-300'}`}>PAID</span>
          </div>
        </div>
          ...
          <div className="space-y-3 animate-in fade-in slide-in-from-top-2 duration-300">
            <div className="space-y-1.5">
              ...
              <input 
                ...
                className="w-full px-3 md:px-4 py-2.5 md:py-3 bg-slate-50 border-none rounded-xl md:rounded-2xl text-xs md:text-sm font-bold text-slate-700 outline-none focus:ring-2 focus:ring-purple-100 transition"
