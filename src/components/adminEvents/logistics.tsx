  ...
  return (
    <div className="bg-white p-6 md:p-8 rounded-[40px] border border-slate-100 shadow-xl space-y-8 font-sans">
      <div className="flex items-center justify-between border-b border-slate-50 pb-4">
        <h3 className="text-slate-900 flex items-center gap-3 text-base  uppercase tracking-widest">
      ...
      <div className="space-y-4 pt-4 border-t border-slate-50">
        ...
        <div className="flex bg-slate-50 p-1.5 rounded-[20px] border border-slate-100">
          <button type="button" onClick={() => onChange('isVirtual', false)}
            className={`flex-1 py-3 rounded-2xl text-xs  transition-all ${!data.isVirtual ? 'bg-white text-purple-600 shadow-sm border border-slate-100' : 'text-slate-400 hover:text-slate-600'}`}
          >PHYSICAL</button>
      ...
      <div className="space-y-4 pt-6 border-t border-slate-50">
        <div className="flex justify-between items-center px-1">
          ...
          >
            <span className={`text-[10px]  tracking-widest ${data.isPaid ? 'text-purple-600' : 'text-slate-400'}`}>{data.isPaid ? 'PAID' : 'FREE'}</span>
        ...
        {data.isPaid && (
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-2">
              <label className="text-[9px]  text-slate-400 uppercase tracking-widest ml-1">Member (UGX)</label>
            ...
            <div className="space-y-2">
              <label className="text-[9px]  text-slate-400 uppercase tracking-widest ml-1">Non-Member</label>
