    ...
    <div className="flex min-h-screen bg-[#F4F7FE] font-montserrat text-gray-900 relative overflow-x-hidden">
      ...
      {deleteModal.isOpen && (
        <div className="fixed inset-0 z-[999] flex items-center justify-center p-4 backdrop-blur-md bg-slate-900/40">
          <div className="bg-white rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-10 max-w-sm w-full shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <h3 className="text-xl md:text-2xl font-bold text-center mb-2 tracking-tight uppercase">Delete Event?</h3>
            <p className="text-gray-600 text-center text-sm mb-8 leading-relaxed">This removes the public post. Registration records remain safe.</p>
        ...
        <div className="flex-1 p-4 md:p-8 lg:p-10 overflow-y-auto">
          <div className="max-w-7xl mx-auto space-y-6">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 md:mb-10 gap-6">
              ...
              {!isEditing && (
                <button 
                  onClick={() => { setSelectedEvent(undefined); setIsEditing(true); }}
            ...
            {isEditing ? (
              <EventForm 
                ...
                onSuccess={() => { setIsEditing(false); fetchEvents(); }} 
                            ...
                            <tr key={event.id} className="hover:bg-slate-50/50 transition-colors">
                              <td className="px-6 py-5">
                                <div className="flex items-center gap-4">
                                  ...
                                  <div className="min-w-0">
                                    <h4 className="text-gray-900 font-bold truncate text-sm">{event.title}</h4>
                              ...
                              <td className="px-6 py-5">
                                <div className="flex justify-center gap-2">
                                  <button onClick={() => { setSelectedEvent(event); setIsEditing(true); }} className="p-2 rounded-lg border border-slate-100 text-slate-400 hover:bg-purple-600 hover:text-white transition-all"><Edit3 size={14} /></button>
                                  <button onClick={() => setDeleteModal({ isOpen: true, id: event.documentId || event.id })} className="p-2 rounded-lg border border-slate-100 text-slate-400 hover:bg-red-500 hover:text-white transition-all"><Trash2 size={14} /></button>
                                </div>
                              </td>
                            </tr>
                          )) : null}
                      ...
                      {filteredEvents.length > 0 ? filteredEvents.map((event) => (
                        <div key={event.id} className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex flex-col gap-4">
                          ...
                          <div className="flex gap-2 pt-2 border-t border-slate-50">
                            <button onClick={() => { setSelectedEvent(event); setIsEditing(true); }} className="flex-1 flex items-center justify-center gap-2 py-3 bg-slate-50 text-slate-600 rounded-xl text-[10px] font-bold uppercase tracking-widest active:bg-slate-100 transition-colors">
                              <Edit3 size={14} /> Edit
                            </button>
