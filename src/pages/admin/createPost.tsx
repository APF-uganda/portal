  ...
  return (
    <div className="flex min-h-screen overflow-hidden">
      ...
      <main className={`flex-1 min-w-0 bg-gray-50 transition-all duration-300 ${collapsed ? "md:ml-20" : "md:ml-64"} min-h-screen flex flex-col`}>
        ...
        <div className="flex-1 bg-[#F4F7FE] px-3 md:px-4 py-3 md:py-6 lg:px-8 overflow-y-auto overflow-x-hidden">
          <div className="w-full max-w-5xl mx-auto">
            ...
            <div className="mb-6 md:mb-8">
              <button
                onClick={handleBack}
                className="flex items-center gap-2 text-gray-600 hover:text-[#5C32A3] transition-colors mb-3 md:mb-4 font-medium text-sm md:text-base"
              >
                <ArrowLeft size={18} className="md:w-5 md:h-5" />
                Back to Forum
              </button>
              
              <h1 className="text-2xl md:text-3xl font-bold text-gray-800 mb-2">Create New Post</h1>
              <nav className="text-xs md:text-sm font-medium text-gray-400 break-words">
            ...
            {/* Success Message */}
            {showSuccess && (
              <div className="bg-green-50 border border-green-200 text-green-700 px-4 md:px-6 py-4 rounded-xl mb-4 md:mb-6 flex items-center gap-3 animate-fade-in">
                <CheckCircle size={20} className="md:w-6 md:h-6 text-green-600 flex-shrink-0" />
                <div className="min-w-0 flex-1">
                  <p className="font-bold text-sm md:text-base">Post created successfully!</p>
                  <p className="text-xs md:text-sm">Redirecting to forum...</p>
                </div>
              </div>
            )}

            {/* Main Form Card */}
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 md:p-5 lg:p-8">
              <div className="mb-4 md:mb-6">
                <h2 className="text-lg md:text-xl font-bold text-gray-800 mb-2">Post Details</h2>
              ...
              {(categoriesLoading || tagsLoading) ? (
                <div className="py-8 md:py-12 text-center">
                  ...
                  <p className="text-gray-500 text-sm md:text-base">Loading form data...</p>
                </div>
              ) : (
                <CreatePostForm
            ...
            {/* Tips Section */}
            <div className="mt-4 md:mt-6 bg-indigo-50 border border-indigo-100 rounded-xl p-4 md:p-5 lg:p-6">
              <h3 className="text-base md:text-lg font-bold text-indigo-900 mb-3">Tips for Creating Great Posts</h3>
